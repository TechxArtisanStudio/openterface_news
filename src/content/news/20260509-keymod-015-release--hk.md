...
> **Beta 説明：** KeyMod Android 應用仍處於活躍 Beta 階段。源碼倉庫尚未公開 — 我們計劃在眾籌成功後正式開源。如果你是 Beta 測試者並需要最新 APK，請在 Discord 上聯繫我們，我們會發送構建給你。
...
KeyMod 應用不僅限於 KeyMod 硬件。現有的 Openterface 用户也可以試用：
...
- **遊戲手柄**：你之前的雙按鈕偏好會在首次啓動時自動激活 **Two buttons** 預設。請使用 **Preset**（輕觸切換，長按查看列表）替代舊的 1 Button / 2 Buttons 控件。
