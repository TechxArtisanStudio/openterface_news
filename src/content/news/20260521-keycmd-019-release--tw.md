...
加上現有的英語、簡體中文、繁體中文、日語、法語、德語和西班牙語，KeyCmd 現已覆蓋我們絕大部分全球用戶群體。
...
- **按鍵點擊預覽**：在點擊之前就能看到將要發送的內容。通過設置啟用。默認啟用。
...
- **頭部右側集群**：改進了主應用和 KM Basic 界面中連接圖標的尺寸
...
KeyCmd 應用不僅限於 KeyMod 硬件。現有 Openterface 用戶也可以試用：
