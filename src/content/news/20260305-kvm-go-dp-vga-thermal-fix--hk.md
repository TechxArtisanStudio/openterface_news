...
## 記錄 1：意識到這不再是「正常温熱」的時刻
在 KVM-GO 開發中期，我們觀察到發熱行為與常見的「小設備會有點熱」預期不符。問題只出現在兩個型號上：DisplayPort（DP）和 VGA。HDMI 型號的温度表現符合預期。

最初症狀很直觀：外殼温度比預期更早達到令人不適的程度。讓我們擔心的不是舒適度，而是內部温度可能已經遠超消費級元器件長期耐受的上限。
...
## 記錄 3：測量真正重要的指標——內部 vs 外部温度
我們決定不再猜測，而是同時測量內外部温度。我們設置了外部和內部監測點，然後進行了長時間負載測試。
...
許多消費級元器件的額定最高工作温度約為 85°C（具體取決於型號和等級）。三位數的內部温度意味着我們面對的不僅是「摸起來燙」的問題，而是可能導致產品壽命縮短或在不同環境下出現不可預測故障的情況。 
...
在重新設計任何東西之前，我們想先做一個快速驗證：如果更快地散走熱量，温度能否顯著下降？

我們用 DIY 風扇做了簡單的強制風冷測試。結果符合物理定律：温度明顯下降，測試中約降了 15°C。這確認了問題是散熱瓶頸，而非測量誤差或軟件行為。

同時這也説明：風扇與我們正在做的產品形態不符。KVM-GO 需要保持緊湊、靜音、一體化。因此強制風冷只作為診斷手段，而不是最終方案。
...
### 步驟 1 之後的變化
散熱有所改善，但我們發現了一個次級問題：熱量仍未能有效傳遞到外殼。部分區域温度偏高，熱成像顯示外殼並未起到足夠的熱 spread 作用。
...
## 結語
這次問題的啓示不僅是「DP 和 VGA 更熱」。多級轉換帶來更高功耗是預期之內的，真正的收穫在於：在這樣小的設備中，熱量往哪裏去和產生了多少熱量同樣重要。

步驟 1（佈局）降低了內部熱點強度。  
步驟 2（機械導熱路徑）讓解決方案穩定且符合產品形態。

沒有風扇，也不需要用户特殊操作，有的只是可預測的、穩定的設計。
