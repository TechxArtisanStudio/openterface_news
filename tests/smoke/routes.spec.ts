...
test('root redirects to English feed', async ({ page }) => {
...
test('KVM-GO product channel loads', async ({ page }) => {
  const response = await page.goto('/en/product/kvm-go/', { waitUntil: 'domcontentloaded' });
  expect(response?.status()).toBe(200);
...
test('pilot article page loads with single h1', async ({ page }) => {
  const response = await page.goto('/en/product/kvm-go/20260518-kvm-go-us-shipment-update/', {
...
test('software and events channels return 200', async ({ page }) => {
  for (const path of ['/en/software/', '/en/events/']) {
    const response = await page.goto(path, { waitUntil: 'commit' });
    expect(response?.status()).toBe(200);
  }
});
